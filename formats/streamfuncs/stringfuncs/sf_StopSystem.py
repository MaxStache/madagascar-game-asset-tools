...
@dataclass
class RW_sf_StopSystem(RW_StreamFunc):
    header: RWHeader = field(default_factory=RWHeader)

    @classmethod
    @override
    def read(cls, parser: Parser) -> "RW_sf_StopSystem":
        sf = cls()

        sf.header = RWHeader.read(parser)
        expect_chunk_type_or_raise(
            sf.header,
            strfunc_func.sf_StopSystem.value,
            "RW_sf_StopSystem chunk type",
        )
    ...
    def write(self, f, stamp):
        ...
        rw_header = RWHeader(
            type=strfunc_func.sf_StopSystem.value,
    ...
    @override
    def streamfunc(self):
        return strfunc_func.sf_StopSystem
