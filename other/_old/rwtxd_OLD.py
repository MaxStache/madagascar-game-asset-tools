...
def _build_xbox_texture_data(tex: NativeTexture) -> bytes:
    ...
    write_u32(buf, PLATFORM_XBOX)
    write_u32(buf, tex.filter_mode)
    _write_fixed_string(buf, tex.name, 32)
    _write_fixed_string(buf, tex.mask_name, 32)
    write_u32(buf, tex.raster_format)
    write_u32(buf, tex.has_alpha)
    write_u16(buf, tex.width)
    write_u16(buf, tex.height)
    write_u8(buf, tex.depth)
    write_u8(buf, tex.mipmap_count)
    write_u8(buf, tex.tex_code_type)
    write_u8(buf, tex.dxt_compression)
    
    total_texel_size = sum(len(mip.texels) for mip in tex.mipmaps)
    aligned_texel_size = (total_texel_size + 3) & ~3
    write_u32(buf, aligned_texel_size)  # texelDataSize xbox field
