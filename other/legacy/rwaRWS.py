...
class AudioStream_2057_StreamHeader:
    ...
    def write(self, f, stamp, data_size):
        buf = io.BytesIO()

        # _unk1 (padded to 4)
        if len(self._unk1) > 4:
            raise ValueError("_unk1 too large")
        buf.write(self._unk1)
        if 4 - len(self._unk1) > 0:
            buf.write(b"\x00" * (4 - len(self._unk1)))
        # sample_rate
        write_u32(buf, self.sample_rate)
        # _unk2 (padded to 4)
        if len(self._unk2) > 4:
            raise ValueError("_unk2 too large")
        buf.write(self._unk2)
        if 4 - len(self._unk2) > 0:
            buf.write(b"\x00" * (4 - len(self._unk2)))
        # stream_size
        write_u32(buf, data_size)
        # bit_depth
        write_u8(buf, self.bit_depth)
        # channels
        write_u8(buf, self.channels)

        # _pad1
        if len(self._pad1) > 2:
            raise ValueError("_pad1 too large")
        buf.write(self._pad1)
        if 2 - len(self._pad1) > 0:
            buf.write(b"\x00" * (2 - len(self._pad1)))

        # _unk_misc_offset (offset from start of stream header chunk)
        misc_offset = self._unk_misc_offset
        if misc_offset == 0 and self.misc_data_size > 0:
            misc_offset = 0x3C
        write_u32(buf, misc_offset)
        # misc_data_size
        write_u32(buf, self.misc_data_size)
        # _unk3
        write_u32(buf, self._unk3)

        # codec_uuid
        write_codec_uuid(buf, self.codec_uuid)
        # codec_uuid_rest
        if len(self.codec_uuid_rest) > 12:
            raise ValueError("codec_uuid_rest too large")
        write_bytes(buf, self.codec_uuid_rest)
        if 12 - len(self.codec_uuid_rest) > 0:
            buf.write(b"\x00" * (12 - len(self.codec_uuid_rest)))

        # input_misc_data
        buf.write(self.input_misc_data)
        # output_misc_data
        buf.write(self.output_misc_data)

        # format_info
        buf.write(self.format_info)
        if 64 - len(self.format_info) > 0:
            buf.write(b"\x00" * (64 - len(self.format_info)))

        # stream_name
        _write_fixed_string(buf, self.stream_name, len(self.stream_name) + 1)
        ...
        padding_needed = (self.header.size + 12) - current_size
        if padding_needed > 0:
...
class AudioStream_2057_Stream:
    ...
    def write(self, f, stamp):
        ...
        self.header_info.write(buf, stamp, len(self.stream_data.data))
        self.stream_data.write(buf, stamp)
    ...
    def export_wav(self, filename):
        ...
        with wave.open(filename, "wb") as wav_file:
            ...
            wav_file.writeframes(self.stream_data.data)
...
class AudioStream_2057_Data:
    ...
    def write(self, f, stamp):
        buf = io.BytesIO()
        write_u32(buf, len(self.subsongs))
        for stream in self.subsongs:
...
class AudioStream_2057:
    ...
    def replace_subsong(self, index: int, new_stream: AudioStream_2057_Stream):
        if index < 0 or index >= len(self.file_data.subsongs):
            raise IndexError("Subsong index out of range")
        self.file_data.subsongs[index] = new_stream
