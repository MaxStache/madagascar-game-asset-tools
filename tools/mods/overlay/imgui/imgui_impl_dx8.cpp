...
void ImGui_ImplDX8_SetupRenderState(ImDrawData* draw_data) {
    ...
    // D3D8 will not accept (or will mishandle -- shows up as blur, not a
    // clean error) a viewport bigger than the actual render target surface,
    // so the viewport and projection below must use the real back-buffer
    // size, not draw_data->DisplaySize (the window's client rect, from the
    // Win32 backend's GetClientRect). This only stays consistent with mouse
    // coordinates (which ARE measured in client-rect space) because
    // SetProcessDPIAware() in dllmain.cpp keeps the client rect and the real
    // back buffer the same size -- without that, client rect and back-buffer
    // size can drift apart under Windows DPI virtualization, which is what
    // caused the earlier cursor-offset bug this same code had.
...
void ImGui_ImplDX8_GetBackBufferSize(float* width, float* height) {
    *width = 0.0f;
    *height = 0.0f;
    ImGui_ImplDX8_Data* bd = ImGui_ImplDX8_GetBackendData();
    if (!bd || !bd->pd3dDevice)
    ...
    if (SUCCEEDED(bd->pd3dDevice->GetRenderTarget(&pSurface)) && SUCCEEDED(pSurface->GetDesc(&desc))) {
